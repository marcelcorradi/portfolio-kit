...
**the owner's** (leave it), never the reverse — it's always safer to skip an update to
...
Pure framework. The owner never hand-edits these, so the new version wins.
...
- `public/robots.txt`, `public/404.html` (but preserve `pathSegmentsToKeep` if the owner
  is on a GitHub-Pages project path — see MIXED).
- The skills: every `SKILL.md` and reference under `.claude/skills/` **except the two
  owner's data files listed below**. Also `.claude/settings.json`,
...
blank); they appear only after the owner works. Leave every one untouched.
...
- **The two owner-data files hiding inside the skills dir — the #1 trap:**
  ...
  - `.claude/skills/portfolio-content/references/profile.md` — identity + accumulated
    case facts.
  These are inside a KIT directory but are OWNER data. Overwriting them erases who the
  owner is. Skip them explicitly.

## MIXED — surgical, never overwrite the whole file

Part kit, part owner. Apply kit changes without clobbering the owner's parts.
...
- **`src/index.css`** — the `@theme inline` block, the full token scale, `@layer base`,
  ...
  `--ring`, light and dark) plus `--font-heading`/`--font-sans` (if the owner picked
  custom fonts) are OWNER. Apply structural changes from the new version, but keep the
  owner's four BRAND values and their font vars exactly.
- **`src/site.config.ts`** — the `SiteConfig` interface, `isConfigured()`,
  `defaultTitle()`, comments = KIT. The exported `site = {…}` values = OWNER. If the new
  version adds a field to the schema, add it; keep every value the owner set.
- **`src/pages/Home.tsx`** — the kit ships a placeholder; the design skill rewrites it.
  Once configured, treat as OWNER: don't overwrite. If the new version changes the
  placeholder meaningfully, mention it, but never replace a built Home.
- **`src/main.tsx`** — routes are KIT, but the owner may have added a route. Don't
  overwrite blindly: show the diff and confirm before applying.
- **`package.json`** — deps, scripts, `version` = KIT. The owner may have added deps.
  **Merge**: bump the kit's dependency ranges and scripts, keep any deps the owner added.
  This is where updated dependency versions land.
- **`src/content/case-visuals/index.ts`** — `getCaseVisuals()` and the `caseVisuals`
  declaration = KIT. The registry's entries (owner's `import`s + registrations) = OWNER.
  Update the surrounding code without dropping the owner's entries.
- **`index.html`** — the two inline `<script>` blocks (SPA redirect, pre-paint theme),
  ...
  OWNER (written by `/setup`). Apply kit changes to the scripts/links; keep the owner's
  meta.
- **`public/favicon.svg`, `public/site.webmanifest`** — ship as kit placeholders carrying
  the brand hex. `/setup` rewrites the hex; the owner likely replaces the favicon mark.
...
- **`vite.config.ts`** — KIT, but `/setup` edits `base` for GitHub-Pages project paths.
  Preserve the owner's `base` value if it isn't `'/'`.

## The rule when a file isn't listed

A new kit version may add files not in this manifest. A **new file that doesn't exist in
the owner's tree** is safe to add (it's new framework). A file that exists in both and
